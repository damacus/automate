...
func (m mockMinio) PutObject(ctx context.Context, bucketName, objectName string, reader io.Reader, objectSize int64,
	...
	if m.ForFailure {
		return minio.UploadInfo{}, fmt.Errorf("error from minio")
	}
...
func TestReportManagerServer_StoreReport_Fail(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			testData2,
			false,
			[]string{"error in storing the report", "error from minio"},
		},
		{
			"large chunk data",
			testData2,
